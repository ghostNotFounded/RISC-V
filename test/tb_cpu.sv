...
module tb_cpu;
    ...
    begin
        ...
        reset = 0;
        // @(posedge clk);
        // $display("  [%s]\nPC=0x%08x instr=0x%08x", name, uut.pc_reg, uut.if_instr);

        // Snoop the store bus for a write to tohost (byte addr 0x1000)
        tohost = 0;
        for (cycle = 0; cycle < 1000; cycle++) begin
            @(posedge clk);
            // if (cycle % 4 == 0) $display("IF/ID Stage:  PC=0x%08x instr=0x%08x", uut.if_id_pc, uut.if_id_instr);

            // if (cycle % 4 == 1) $display("ID/EX Stage:  PC=0x%08x rs1=%d rs2=%d rd=%d PCSel=%b stall=%b flush=%b Y=0x%08x imm=0x%08x A=0x%08x B=0x%08x Y=0x%08x", uut.id_ex_pc, uut.id_ex_rs1, uut.id_ex_rs2, uut.id_ex_rd, uut.id_ex_pcsel, uut.stall, uut.flush, uut.ex_mem_y, uut.id_ex_imm, uut.ex_alu_a, uut.ex_alu_b, uut.ex_y);

            // if (cycle % 4 == 2) $display("EX/MEM Stage: PC=0x%08x flush=%b", uut.ex_mem_pc, uut.flush);

            // if (cycle % 4 == 3) $display("MEM/WB Stage: PC=0x%08x rd=%d flush=%b WBSel=%02b WBData=0x%08x\n", uut.mem_wb_pc, uut.mem_wb_rd, uut.flush, uut.mem_wb_wbsel, uut.wb_data);


            if (uut.RF.regs[3] != 0 && uut.RF.regs[3][0] == 1'b1) begin
                repeat(5) @(posedge clk);
                tohost = uut.RF.regs[3];
            break;
        end
        end
